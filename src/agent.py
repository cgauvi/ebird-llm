...
1. When an eBird data tool returns results, the output includes:
   ...
   - A line "JSON file: <full path>" with the EXACT path the server just wrote
   - A prompt: Call create_sightings_map / create_historical_chart / show_observations_table
     with observations_file="<that exact path>"
2. Use the compact summary for reasoning and your text response to the user.
3. When calling a viz tool, pass the file path from step 1 as observations_file —
   copied character-for-character, including the timestamp. File-based loading
   avoids re-passing JSON through the LLM.

NEVER FABRICATE A FILE PATH (CRITICAL):
- The path's timestamp portion (e.g. 20260428_125049_794403) is generated by the
  server using microsecond precision. You CANNOT guess it. You MUST copy it from
  the most recent eBird tool output in THIS turn.
- If you do not have a "JSON file: ..." line from a tool output earlier in the
  current turn, you have no valid path. Either: (a) call an eBird data tool
  first and use the path it returns, or (b) omit the observations_file argument
  entirely so the tool falls back to the session cache. Do NOT invent a path
  that "looks like" the format — the file will not exist and the call will fail.
- If a viz tool returns "observations_file=... does not exist on disk", the
  error message names the only valid path. Retry with that exact string and
  do not modify the timestamp.

Workflow guidelines:
1. When a user asks about sightings, ALWAYS fetch data first with an eBird tool.
   The viz tools refuse to use cache data left over from a previous turn — you
   MUST fetch fresh in the current turn before asking for a map/chart/table.
...
3. After fetching observation data, proactively offer to (or automatically) call
   ...
   When the user names specific species (e.g. "map Bald Eagle and Osprey")
   AND the fetched data contains other species too, you MUST pass
   species_filter=["Bald Eagle", "Osprey"] (codes or common names both work)
   to the viz tool. The viz tools render every record in their input —
   they do not infer the user's intent from the conversation. Pass
   species_filter=["*"] (or omit it) when the user wants every species.
            ...
            for msg in chunk["agent"]["messages"]:
                if hasattr(msg, "tool_calls") and msg.tool_calls:
                    ...
                    if stripped.startswith("[") or (
                        ...
                        if (
                            VizBuffer["type"] is None
                            and not _is_clarifying_question(stripped)
                            and obs_cache_is_current_turn()
                        ):
